...
- Added automatic proactive Rescue routing alongside explicit `$zcode:rescue`, with authoritative explicit choices, semantic objective normalization, active-child rejoin, and no `--auto` flag. Root now performs a single private stdin prepare rollout into exact-bound, expiring, single-consume prepared state; task-blind named and generic children run only `invoke-prepared rescue`.
