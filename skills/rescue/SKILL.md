...
Background placement (`options.execution` was `background`) is the second exception: once the preparation is accepted and the operation launched, Root returns the launch confirmation immediately — the `rescueChildPath` task handle and the `/zcode:status` and `/zcode:result` query commands; the reserved job identifier, its terminal status, and the Host Completion Notice arrive later through the notification relay — and ends the turn WITHOUT entering the `wait_agent` loop. The Host-managed child continues to terminal on its own, and a missed delivery is emitted on the next UserPromptSubmit. Do not poll `wait_agent`, `list_agents`, or `/zcode:status` on the user's behalf for background placements; `$zcode:status` and `$zcode:result` remain available whenever the user asks.
