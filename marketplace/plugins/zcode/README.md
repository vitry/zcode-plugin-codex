...
`--resume` remains an argument-free public choice: it accepts no child, session, job, binding, operation, or handle argument. For an exact continuation, Root privately retains the unchanged exact host child ID and agent path pair from one linked spawn/start lifecycle and sends it only through the private preparation protocol. The pair is a selector that only narrows selection, not authority: complete binding, original session, permission, and canonical workspace validation remain mandatory. A targetless resume with multiple usable bindings is still ambiguous and must fail closed; the plugin never ranks or substitutes a sibling.
