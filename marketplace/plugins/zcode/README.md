...
Cold resume has one narrow compatibility recovery. Only after `session/resume` returns the exact snapshot warning `ZCODE_RUNTIME_MODEL_UNAVAILABLE`, and only when neither an explicit `--model` nor the workspace model policy supplies a tuple, the plugin lazily reads the effective `HOME` file `.zcode/cli/config.json` and selects its `model.main`. Precedence is explicit `--model`, workspace model policy, then CLI `model.main`. It calls `session/setModel` exactly once even when the tuple text already matches, applies requested effort only after materialization, and performs one `session/send`. A warm resume does not read config or issue that recovery setModel. Missing or invalid config, an unsupported model, setModel rejection, and later send failure remain visible failures: there is no fresh fallback, prompt resend, replacement session, retry loop, or CLI-config mutation.
