...
Legacy jobs-only state may adopt the exact eligible continuation candidate once; ambiguous or previously pending legacy state is rejected instead of guessed. A permission change cannot resume the old binding, while `--fresh` captures the current permission snapshot. Explicit cancel, invalidation, and revoked same-child replacement remain terminal and cannot be migrated. An invalid binding, executor mismatch, wrong workspace, closed session, or inconsistent provenance must fail closed without latest-session fallback. Because the managed Role bytes changed, `role-status rescue` can report `upgrade-required`; rerun `$zcode:setup` before continuing.
...
SessionEnd is runtime ownership loss, not binding revocation: it settles writable jobs and cleans runtime/preparation resources while exact resumable bindings remain available to the same Root session. Rescue states are resident, unloaded but resumable, completed but resumable, or revoked. Mapping is exact child-scoped by parent session, canonical workspace, child identity/path/Role, and operation; ambiguity fails closed. A fresh operation on a different child preserves the first binding, while explicit same-child replacement may supersede only that child. `WRITABLE_JOB_EXISTS` remains a separate conservative writable-exclusion policy and does not promise parallel writable Rescue operations.
