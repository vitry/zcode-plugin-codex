...
Require a non-empty normalized business objective. Rescue may change the workspace. Select the placement before preparation without asking the user another placement question: an explicit `--wait` or `--background` flag is authoritative, and otherwise task complexity decides — a task that is small and clearly bounded is placed in the foreground, while a task that is multi-step, open-ended, or likely long is placed in the background. Announce an inferred background placement in the same launch confirmation without asking for confirmation; an inferred foreground placement needs no announcement. The selection is recorded only as the private `execution` enum, which is only `foreground` or `background`; placement itself never adds task text or a private identifier to any child-facing boundary, and both placements start the same constant `invoke-prepared rescue` child command. Before the child starts, the parent must prepare the exact private Rescue envelope as specified below. Never place user text, command arguments, job or session identity, permissions, credentials, or authorization material in a process command or agent message.
...
The payload for every new flow is the exact version-3 envelope with keys `version`, `source`, `task`, `options`, and `continuationTarget`:

```json
{"version":3,"source":"explicit","task":"<normalized non-empty business objective>","options":{"execution":"foreground","resume":"fresh","model":"<model>","effort":"<effort>"},"continuationTarget":null}
```

New flows always emit version 3. Versions 1 and 2 are accepted only for compatibility and are never emitted by these instructions. `source` is exactly `explicit` or `proactive`; `task` is the normalized non-empty objective. `options` permits only existing `execution`, `resume`, `model`, and `effort` fields. Omit every absent option; never encode an absent option as null. `execution` is only `foreground` or `background`, `resume` is only `fresh` or `resume`, and model and effort retain the existing public meanings.
...
Background placement (`options.execution` was `background`) is the second exception: once the preparation is accepted and the operation launched, the mapped command durably reserves the job, launches one detached session-bound runner, and returns only a queued acknowledgement before exiting — the child does not observe the run to terminal. Root returns that child stdout verbatim — the queued acknowledgement naming the reserved job identifier and its `$zcode:status` and `$zcode:result` query commands — and ends the turn WITHOUT entering the `wait_agent` loop. Queued means accepted for execution only: it never proves the runner claimed the job, started work, or remains alive, and the acknowledgement never finalizes a completion marker. The detached runner continues on its own; the terminal outcome is discovered through `$zcode:status` and `$zcode:result`, and an unread terminal result is announced at the next UserPromptSubmit. Do not poll `wait_agent`, `list_agents`, or `/zcode:status` on the user's behalf for background placements; `$zcode:status` and `$zcode:result` remain available whenever the user asks.
