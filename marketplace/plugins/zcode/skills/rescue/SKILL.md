...
Require a non-empty normalized business objective. Rescue may change the workspace. Select both placement dimensions before preparation without asking the user another placement question: an explicit `--background` or `--wait` flag is authoritative and mutually exclusive, and otherwise task complexity decides. The four authorized mappings are exactly:

- Explicit `--background`: Host placement is `background`; Companion execution is `foreground`.
- Explicit `--wait`: Host placement is `foreground`; Companion execution is `foreground`.
- No flag, small and clearly bounded: Host placement is `foreground`; Companion execution is `foreground`.
- No flag, complex, open-ended, multi-step, or likely long: Host placement is `foreground`; Companion execution is `background`.

`hostPlacement` is exactly `foreground` or `background`, and `companionExecution` is exactly `foreground` or `background`; neither enum implies the other. Host placement is the only input to Root waiting: a Host `foreground` parent joins only the exact child with the longest native `wait_agent` until terminal stdout or the queued acknowledgement, and a Host `background` parent performs no `wait_agent` in the initiating interaction and reports only that the Host child was launched. Companion execution is the only input to Companion routing: `companionExecution: background` alone starts the detached runner, the child returns queued, and Root does not poll `$zcode:status` or `$zcode:result` afterward; `companionExecution: foreground` never creates a detached runner. Root announces expected Companion `background` execution from a complex no-flag inference in the same launch confirmation without asking for confirmation; Companion `foreground` execution needs no announcement. Placement never adds task text or a private identifier to any child-facing boundary, and both placements start the same constant `invoke-prepared rescue` child command. Before the child starts, the parent must prepare the exact private Rescue envelope as specified below. Never place user text, command arguments, job or session identity, permissions, credentials, or authorization material in a process command or agent message.
...
The exact ready object has only `type`, `role`, `status`, and `continuation`: `{"type":"role-status","role":"zcode-rescue","status":"ready","continuation":{"state":"none"}}`. The nested `continuation` object has exactly one key, `state`, whose only allowed values are `none`, `present`, and `blocked`; reject extra fields. Non-ready output keeps its existing contract. This companion-owned observation is scoped to the current parent and execution workspace, reveals no private identifiers, and never grants preparation or write authority.

- `none` proves no current-parent continuation evidence: ordinary project continuation materializes `fresh` with `continuationTarget: null`, preserving the current worktree and progress without asking permission to avoid a nonexistent resume. Project continuity is not operation identity: “continue this project” or “continue T1” does not by itself require the same Rescue operation.
- `present` means current-parent evidence exists, not proof that any operation is resumable. Apply the retained exact-child selector and same-parent validation below; multiple bindings are not automatically blocked when the exact selector is retained.
- `blocked`, missing, or invalid continuation observation is not `none`. For inferred continuation, explain the failed observation and stop before preparation; do not claim resume is available or infer fresh. Never reconstruct a selector from memory artifacts, project files, list order, or latest-session history.
- Explicit `--resume` and explicit same-operation requirements remain authoritative and never silently fall back to fresh. With `none`, explain that the required continuity is unavailable before preparation. Clear independent requests and explicit `--fresh` retain the fresh route even with a blocked advisory; actual preparation and write guards remain authoritative.

Only after this observation, apply stopped-child and independent-operation routing:

- **Stopped exact same-operation child.** Root expresses continuation through the preparation envelope. For every successful `spawn_agent` call, retain the exact canonical path in the returned result's `task_name` with that logical Rescue operation. Retain it unchanged across follow-up, stop, restoration, and later turns. Root does not read, inspect, or depend on internal activity records and never needs a child ID; the child ID remains internal plugin identity. Never guess, derive, manufacture, or synthesize the path from `taskName`, a suffix, list order, a timestamp, nickname, display name, or any other presentation value. Root chooses the intended logical operation from its conversation and linked lifecycle history, then supplies that operation's retained canonical path only as the private `continuationTarget`. The companion discovers host children, obtains the selected host child ID internally, joins the selected child to private stopped-executor provenance, validates the complete durable binding, and returns a prepared route to the exact persisted child. With `present`, a proactive clear same-operation continuation materializes `resume` as `resume`. For an explicit no-choice request, omit `resume` and use a null target only when the total retained stopped operations is exactly one and it is the sole semantic candidate; the exact child may then return `needs-choice`, after which Root uses the existing same-child choice protocol. Explicit `--resume` is authoritative. If the intended operation is ambiguous or its exact retained canonical path is unavailable, ask the user for clarification and do not prepare or invoke while guessing or without that clarification. Never inspect rollout JSONL or private bindings and never ask the user for a child ID. The prescribed follow-up reactivates the original stopped child and history without a second `SubagentStart` and with zero spawn calls. A missing, closed, corrupt, invalid, or mismatched binding, session, workspace, executor, host child, or durable provenance must fail closed without choosing a latest session, falling back to another child, or spawning.
...
Apply this precedence after the parent continuation observation and before preparation. The `none` and failed-observation rules above take precedence over inferred continuation and the explicit no-choice candidate branches below:
...
- With one semantic candidate and total retained stopped operations more than one, ask exactly once before prepare whether to resume or start fresh. A resume answer uses that candidate's exact retained canonical path; a fresh answer uses `continuationTarget: null`.
- For an explicit request with no choice (neither `--fresh` nor `--resume`) and more than one semantic candidate, ask exactly once before prepare, followup, or spawn. One answer must simultaneously resolve both the operation and `resume` or `fresh`: the answer chooses resume with one logical operation's exact retained canonical path, or the answer chooses fresh with `continuationTarget: null`. Do not split this into an operation question followed by a resume/fresh question.
...
- For a proactive clear continuation with `present`, materialize `resume` as `resume` in the prepare envelope. If its exact retained canonical path is unavailable, clarify or fail closed; never use fresh/null as a fallback. For a proactive clear independent task, materialize `resume` as `fresh` in the prepare envelope. A proactive clear route must include either `fresh` or `resume`.
...
After Role readiness and continuation observation above, apply entry classification and choice precedence before starting private preparation.
...
The payload for every new flow is the exact version-4 envelope with keys `version`, `source`, `task`, `options`, and `continuationTarget`:
...
New flows always emit version 4. `source` is exactly `explicit` or `proactive`; `task` is the normalized non-empty objective. `options` permits only `hostPlacement`, `companionExecution`, `resume`, `model`, and `effort` fields, and both placement fields are required in every new preparation. Omit every absent option; never encode an absent option as null. `hostPlacement` is exactly `foreground` or `background`, `companionExecution` is exactly `foreground` or `background`, `resume` is only `fresh` or `resume`, and model and effort retain the existing public meanings. Only the three matrix mappings above are ever emitted; Host `background` with Companion `background` has no authorized mapping and fails closed. Version 3 remains the only accepted legacy envelope and stays read-compatible with its historical coupled `options.execution` meaning; updated instructions never emit it. Versions 1 and 2 private preparation envelope compatibility is removed: those envelopes are rejected and never accepted.

`continuationTarget` is either `null` or the exact one-key canonical-path selector `{"agentPath":"<retained spawn result task_name>"}`. Fresh, independent, non-resume, and compatibility-choice preparation always uses `continuationTarget: null`. An exact resume sets `continuationTarget` to the unchanged retained canonical path. A non-null target is valid only with `options.resume` equal to `resume`.

Task, source, and options are allowed only in the parent `write_stdin` payload. The continuation target is allowed only in the single post-readiness `write_stdin` frame. The serialized selector must never enter argv, the environment, a spawn message, assignment, output, child transcript, relay, status, result, task name, agent metadata, logs added by the plugin, or ZCode requests. In particular, never propagate `continuationTarget` or any child ID into a public or child-facing boundary. The independently validated agent path may later appear alone only as the plugin's existing prepared follow-up route target. The prepared state is private and task-free at every child-facing boundary.
...
The named assignment is exactly `Run the installed prepared ZCode Rescue forwarder now. Return its public stdout verbatim.`. The generic assignment is the complete fixed generic message below after its one immutable launcher substitution. The target is the plugin-prescribed agent path, not a child ID guessed or selected by Root. The plugin route remains the only follow-up authority: Root must follow `prepared.route.target` and must never directly follow its retained path, target, or handle. A host rejection is terminal for this preparation and must not fall back to a spawn.
...
Keep the exact returned `task_name` as `rescueChildPath`; that returned `task_name` is both the active logical handle and canonical continuation path for the operation. It is not a Codex child thread ID and is never placed in a preparation as an ID. While that operation remains selected, do not call `spawn_agent` again after `rescueChildPath` exists. Wait for that same child to reach a terminal or idle state. A wait timeout, early return, or ordinary user steering does not authorize a new child or a second execution; continue only with the same `rescueChildPath`. Call `wait_agent` again as appropriate and rejoin that same child. Retain the canonical path unchanged across stop, restoration, follow-up, and a later exact resume. The sole exception is the exact consumed pending-fresh `parent-replan` outcome below, which ends selection of the old child before the parent prepares and spawns the independent operation.

Host placement alone decides Root waiting, and the wait decision never inspects `companionExecution`. With Host `foreground` placement, Root joins the exact child with the longest native `wait_agent` until the child returns terminal stdout or the queued acknowledgement. With Host `background` placement, Root starts or follows the exact child, performs no `wait_agent` in the initiating interaction, and returns a bounded acknowledgement claiming only that the Host child was launched — never that Companion work was queued, accepted, started, or completed — relying on native child completion/error delivery. With Companion `background` execution, the mapped command durably reserves the job, launches one detached session-bound runner, and returns only a queued acknowledgement before exiting — the child does not observe the run to terminal — and Root does not poll `$zcode:status` or `$zcode:result` afterward. Root returns that child stdout verbatim — the queued acknowledgement naming the reserved job identifier and its `$zcode:status` and `$zcode:result` query commands. Queued means accepted for execution only: it never proves the runner claimed the job, started work, or remains alive, and the acknowledgement never finalizes a completion marker. The detached runner continues on its own; the terminal outcome is discovered through `$zcode:status` and `$zcode:result`, and an unread terminal result is announced at the next UserPromptSubmit. `$zcode:status` and `$zcode:result` remain available whenever the user asks.
...
Use this wait shape, then select only the result or status belonging to `rescueChildPath`:

```text
wait_agent({ timeout_ms: 600000 })
```

Use the longest permitted native wait if active bounds or higher-priority instructions require a different interval. Do not use periodic sleep, list_agents, or status calls solely to check liveness. On timeout or unrelated mailbox activity, continue waiting for the same rescueChildPath. Inspect the exact child only when route discovery or lifecycle reconciliation requires evidence. Do not expect ordinary progress messages; native child completion/error delivery and the original execution remain authoritative. Companion `background` acknowledgement ends runner supervision as specified above.

Accept an unexpected message only when its author is the exact `rescueChildPath`. Such an update from the exact `rescueChildPath` is liveness only: show it if useful, then wait or rejoin that same child. A progress update is never completion, never terminal evidence, and never authority to spawn, follow up, execute a companion command, or change ownership. Reject sibling-authored, arbitrary, detailed, or malformed progress. On terminal completion proven by the original child execution, return only the child's public stdout verbatim without interpretation.

A project tool, test, build, lint, or other command failure observed while the ZCode turn remains active is not a Rescue failure. Root must continue waiting for the exact `rescueChildPath`. Do not hard-code project commands or parse their output to infer completion. Only the original companion and ZCode authoritative terminal result may end the operation.

Detailed semantic progress belongs only to the child transcript and durable job preview; routine progress and heartbeat messages do not reach the parent. When explaining inspection, direct the user to `/agent` or `/subagents` to select the Rescue child; `/ps` lists background terminals for the currently active thread and is not a subagent selector. Online conversation subscription may degrade to fixed lifecycle messages and the 20-second heartbeat without changing the authoritative result. Command and query previews are control-free single lines shortened to 96-character display bounds, but truncation is not secret redaction; never claim it removes secrets supplied in a command or search.

The named and generic routes use the same same-child choice continuation for resume and the same one-shot pending consumer for fresh. If that child returns a public `needs-choice` response, preserve that response verbatim and ask the user exactly once; do not choose for the user. Immediately after the verbatim stdout, append exactly `Choose resume or fresh.` and no other text. Retain the original `rescueChildPath` across that user turn. A non-choice reply or ordinary steering must not be forwarded to the child and must not cause another question, spawn, or execution. After the user supplies one unambiguous choice, set `continuationMessage` to exactly one of these strings, with no prefix, suffix, interpolation, or additional field:
...
```text
followup_task({ target: rescueChildPath, message: continuationMessage })
```

For `resume`, wait again and inspect only that same `rescueChildPath`; never spawn, retry, or execute a companion command in the parent. The child response is authoritative. Present success stdout verbatim.

For `fresh`, the old child must return exactly `{"type":"parent-replan","command":"rescue"}` after consuming the pending record, with no ZCode or binding action. This outcome is not a Rescue result and must not be presented as final. Retire the old `rescueChildPath`, return to the parent preflight and private preparation flow with the retained original objective and `resume` materialized as `fresh`, and obey only the newly prepared collision-free `spawn` directive. Start that one new child and wait for its authoritative result. Never run fresh inside the old child, follow it as the fresh operation, or reuse its binding.
