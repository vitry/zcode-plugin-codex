...
插件的 Companion 在 preparation 期间、任何替代 spawn 之前，通过 app-server 的精确 parent relationship query 发现该 parent 的持久化 Codex children。全局 thread listing 会漏掉 preview 为空的已恢复 child，因此绝不用它证明某个 spawn 名称空闲。如果已安装的 Codex 版本不能证明支持精确 parent API，preparation 会 fail closed，不会重试或猜测 spawn。具有 executor 证据的已停止 child 继续走普通 Hook-proven 恢复路径。为了兼容旧版插件，一个精确、未加载、direct、具名 `zcode-rescue` 且位于受管路径的 child，即使历史 Hook executor 已不存在，也可以被一次性 adoption。插件会按精确 Codex ID 再读该 child，并在预留任何 job 前重新证明当前 parent turn、origin 到 execution worktree 的链接、permission 快照与一次性 preparation。adoption 会被显式记录；插件绝不会伪造历史 `SubagentStart`、executor 或 route。generic `default` 兼容 child 仍必须具有真实 executor provenance。无关的 `default` 与 `explorer` children 只占用路径，既不能阻断有效 Rescue candidate，也不能授予 follow-up 权限。证据有歧义或相互矛盾时会 fail closed。协议版本按 action 区分：follow-up directive 是 strict version two 并携带固定 assignment；spawn directive 保持 strict version one 且不含 assignment，从而保留 named-to-generic schema negotiation。Root 执行已选择且不含 task 的 directive，但不负责发现或选择 child identity。活动的 Rescue child 仍通过现有路径重新加入。30 分钟 age 与名称或路径碰撞都不构成授权或权威，也不能授权恢复或替代创建。
