...
Rescue 有两种等价入口：显式 `$zcode:rescue` 是请求中字面且适用的入口；Root 也可以根据完整业务目标主动选择 Rescue。这就是自动路由，不提供 `--auto` 选项。显式 `--fresh` 或 `--resume` 始终权威；推断 fresh/resume 前，Root 先读取 companion 提供的当前 parent 续做观察。确认没有证据（`none`）时，普通“继续项目/T1”会在当前 worktree 创建独立 fresh operation，保留已有项目进度，不会恢复其他 parent 的 operation。有证据（`present`）仍需保留的精确 child selector 和完整 binding 验证；观察失败（`blocked`）不等于不存在。明确要求同一 operation 时绝不静默回退 fresh；明确独立任务和显式 `--fresh` 保持 fresh 路由，但仍受 prepare 与写入 guard 约束。Rescue 的 placement 由两个独立层决定：Host placement 只决定 Root 是否在发起交互中加入 Rescue Child，Companion execution 只决定 child 是持有一个附着到终态的 Companion 进程，还是在启动 session-bound detached runner 后返回 queued。Host background 指 Root 不会在发起交互中加入 Rescue Child；Companion background 指 Rescue Child 在启动 session-bound detached runner 后返回 queued。四种授权映射是：显式 `--background` 映射为 Host background 加 Companion foreground；显式 `--wait` 映射为 Host foreground 加 Companion foreground；无标志的小而边界清晰任务映射为 Host foreground 加 Companion foreground；无标志的复杂、多步、开放或可能耗时任务映射为 Host foreground 加 Companion background。没有显式标志时，Root 对 placement 的推断只按任务复杂度执行，不追问额外的 placement 问题，并在启动确认中宣布 Companion background；显式 `--wait` 与 `--background` 始终权威，优先于该推断。

`--resume` 仍是无参数的公开选择：它不接受 child、session、job、binding、operation 或 handle 参数。对于精确续做，Root 私下保留成功 `spawn_agent` 调用返回的精确 `task_name`，作为该 operation 不变的 canonical agent path。Root 发送 private version-4 preparation，其中只以 `continuationTarget: {"agentPath":"/root/..."}` 携带该 path；插件在精确 parent 的持久 child graph 内部解析 host child ID。canonical path 是只收窄选择的 selector，不是 authority：完整 child-ID/path binding、原始 session、permission 与 canonical workspace 验证仍全部是强制要求。存在多个合格 sibling 时，精确 version-4 selector 只解析该 canonical agent path 上的 sibling，并且只恢复该 sibling 的完整 binding 与原始 ZCode session。targetless/null 兼容路由仅在唯一 complete binding 合格时允许。新流程总是发出 version 4；envelope version 3 仍是唯一保留读取兼容的旧 envelope，保持其历史耦合 placement 语义，且绝不被发出；envelope version 1 与 2 不再被接受。不带 target 且存在多个可用 binding 的 resume 仍属歧义并 fail closed；插件绝不排序或替换 sibling。

Root 在 raw-capable TTY 上启动 `prepare rescue`。companion 先启用 raw mode，再输出精确且不含 task 的 readiness；readiness 是非终态。只有看到该行之后，Root 才通过私有 stdin 发送一行 JSON，并以 LF 结尾；不发送 EOF 或 U+0004。companion 消费这一帧、恢复 raw mode，并提交绑定精确 session、turn、workspace 和 executor 的 prepared state。非 TTY 或 raw mode 失败会在 task 交付前停止，且不会 spawn child。tool output 绝不包含或回显 payload；返回边界只有不含 task 的 readiness 和最终 prepared 确认。随后具名 Role 或 generic child 都只运行常量 `invoke-prepared rescue` forwarder，不接收 task、options、capability 或授权材料。若已经有活动的 `rescueChildPath`，它就是成功调用 `spawn_agent` 返回的精确 canonical `task_name`；Root 会重新加入并等待该 path 上的 Rescue child，不会重复 preflight、prepare、spawn 或 invoke。Root 绝不会收到或保留 child ID；插件会从精确 parent 的持久 child graph 内部解析它。

Companion 只通过 app-server 的精确 parent relationship query 发现持久化 Codex children。已有 child 只有凭完整的 exact binding 才合格；host-only 或 jobs-only 历史绝不能被采用。惰性兼容迁移仅限唯一的精确 v1/v2 `closed/session-ended` binding。独立进程观测到 `notLoaded` 只有在 parent、child ID、精确 path、Role/type、canonical workspace、binding、jobs、operation、generation、permission 与原始非空 `zcodeSessionId` 全部一致时才可接受。因此，有精确 private selector 时，即使存在多个合格 sibling binding，`/root/zcode_rescue_task_2` 也只 resume 自己的 session；只有 targetless 路由要求它是唯一合格 binding。没有精确 private selector 时，两个可用 binding 属于歧义并 fail closed。base path、latest job/session、时间戳、列表顺序和后缀排序绝不选择候选。follow-up directive 仍是带固定 assignment 的 strict version two；spawn directive 仍是不含 assignment 的 strict version one。
...
Rescue 在两个 placement 层都保持 host 托管且 session-bound，detached runner 只属于 Companion background。显式 `--background` 是 Host background 加附着的 Companion foreground：Root 启动 Rescue Child 但不在发起交互中加入它，child 持有一个附着的 Companion 进程直到终态，Root 依赖原生 child 完成/错误投递。正常 durable job 仍会通过前台预留路径为 lifecycle、Status/Result、取消与恢复预留，但该运行是附着的：不存在 detached/queued 的 runner job，不写入 `rescueRunnerVersion` 标记，也不返回 queued 确认。Companion background 只来自无标志复杂任务推断：Rescue Child 持久预留一个精确 job，启动一个 session-bound 的 detached runner，随后返回 queued 确认再退出；新的 Rescue 也绝不复用旧 detached worker 启动路径。queued 仅表示已被接受等待执行：它不证明 runner 已 claim 或已开始运行，queued 确认也绝不会最终化完成标记。runner 会先发布 result artifact 与持久终态 winner；`$zcode:status` 与 `$zcode:result` 仍是权威的拉取接口，未读的终态结果会在下一次 UserPromptSubmit 依据现有单次送达 claim 通告，完整输出仍通过 `$zcode:result` 获取。持久恢复继续使用 `$zcode:status`、`$zcode:result` 和 `$zcode:cancel`。普通 steering、等待超时或父/child 丢失都不授权替代执行。
...
历史 detached Rescue job 仍保留完整的持久操作面：原 owner 依旧可以通过 `$zcode:status`、`$zcode:result` 和 `$zcode:cancel` 检查、等待、取消并收敛这些任务（含 PromptSubmit 回退），而新 Rescue 绝不重新启动旧 detached worker 路径。升级绝不批量取消历史任务，也绝不把它们收编为新的 host child 或重新启动 detached worker。
...
后台任务有独立生命周期：启动它的前台命令或 Codex turn 结束时，后台任务不会自动取消。但所属 Codex session 结束时不同：`SessionEnd` 会停止 Host background 的 Rescue 与 Companion background 的 detached run 并做持久结算，同时也会发现并结算活动中的 detached 只读 Review 或 Adversarial Review 运行，因此只读后台工作同样不会在登出后继续存在。用 `$zcode:status <job-id>` 查看，用 `$zcode:cancel <job-id>` 显式取消；ownership 仍只属于预留该 job 的 Codex session。
