...
`--resume` 仍是无参数的公开选择：它不接受 child、session、job、binding、operation 或 handle 参数。对于精确续做，Root 私下保留来自同一个已关联 spawn/start lifecycle 的不变精确 host child ID 与 agent path 对，并且只通过私有 preparation 协议传递。这对值是只收窄选择的 selector，不是 authority：完整 binding、原始 session、permission 与 canonical workspace 验证仍全部是强制要求。不带 target 且存在多个可用 binding 的 resume 仍属歧义并 fail closed；插件绝不排序或替换 sibling。
