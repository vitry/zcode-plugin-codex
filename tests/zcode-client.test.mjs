...
test('snapshot identity binding rejects cross-workspace and nested session aliases', async (t) => {
  for (const variant of ['wrong-workspace', 'wrong-projection-session', 'wrong-session-target', 'wrong-projection-target', 'wrong-message-session', 'wrong-part-session']) {
...
test('broker validates snapshot workspace and every current-session ID before committing ownership', async (t) => {
  for (const variant of ['missing-workspace', 'wrong-workspace', 'wrong-projection-session', 'wrong-session-target', 'wrong-projection-target', 'wrong-message-session', 'wrong-part-session']) for (const explicit of [false, true]) await t.test(`${variant}-${explicit ? 'explicit' : 'anonymous'}`, async () => {
