...
test('broker conversation admission rejects and cleans every safe malformed or duplicate acknowledgement', async (t) => {
  const safeInvalid = [
    ...
    { ack: { subscriptionId: 'x'.repeat(257), mode: 'snapshot', logEpoch: 'epoch-1' } },
  ];
...
test('owner release keeps its stop fence through durable ownership commit', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'zcode-broker-release-commit-fence-')); const endpoint = join(directory, 'broker.sock'); const ownershipPath = `${endpoint}.owners.json`; const ownerId = 'release-commit-owner'; const sessionId = 'release-commit-session'; const writes = []; let sendCalls = 0;
  const socket = { writable: true, destroyed: false, zcodeWriter: { write: (line) => writes.push(JSON.parse(line)) }, destroy() {} }; const broker = newTestBroker({ endpoint, brokerToken: 'b'.repeat(64), workspace: directory, launch: { command: process.execPath, args: [fixture], target: fixture } }); broker.authenticated.add(socket); broker.socketOwnerIds.set(socket, ownerId); broker.sessionOwners.set(sessionId, { ownerId, socket, claimToken: null }); broker.activeSessionSockets.set(sessionId, { socket, token: 'release-active', baseline: 1, inputId: 'release-input' }); broker.activeSessions.add(sessionId); await writeFile(ownershipPath, JSON.stringify({ version: 1, sessions: { [sessionId]: ownerId } })); broker.ownershipStoreEstablished = true; broker.protocol = { request: async (method) => { if (method === 'session/send') sendCalls += 1; return {}; }, cancelTurn() {} };
  ...
  const releasing = broker.releaseOwner(socket, ownerId, []); await new Promise((resolvePromise) => setTimeout(resolvePromise, 40)); const fencedDuringCommit = broker.stoppingSessions.has(sessionId); const sending = broker.handleLocal(socket, JSON.stringify({ id: 91, method: 'session/send', params: { sessionId, inputId: 'new-input', content: 'must reject' } })); await new Promise((resolvePromise) => setTimeout(resolvePromise, 20)); assert.equal(sendCalls, 0);
  releaseLock(); await holder; const released = await releasing; await sending; assert.equal(fencedDuringCommit, true); assert.deepEqual(released.releasedSessionIds, [sessionId]); assert.equal(broker.sessionOwners.has(sessionId), false); assert.equal(broker.activeSessionSockets.has(sessionId), false); const rejection = writes.at(-1).error; assert.ok(rejection.code === -32041 || rejection.data?.pluginError?.code === 'ZCODE_TURN_ACTIVE'); await rm(directory, { recursive: true, force: true });
});
