...
test('a stale malformed direct unsubscribe acknowledgement cannot add old-generation evidence', async () => {
  ...
  assert.equal(broker.protocol, replacementProtocol); assert.equal(broker.orphanedConversationSubscriptions.size, 1); assert.equal(broker.orphanedConversationSubscriptions.get(replacementTombstone.key), replacementTombstone); assert.equal(oldCloseCalls, 0); assert.equal(writes.find((frame) => frame.id === 81)?.result, undefined); assert.equal(broker.admission.activeCount, 0); await rm(directory, { recursive: true, force: true });
});
...
test('three hundred malformed unsubscribe acknowledgements cannot bypass the orphan cap', async () => {
  ...
  for (let index = 0; index < 300; index += 1) {
    const sessionId = `malformed-unsubscribe-session-${index}`; broker.sessionOwners.set(sessionId, { ownerId, socket, claimToken: null }); const subscriptionId = `malformed-unsubscribe-${index}`; const connectionId = `malformed-connection-${index}`; const protocol = { request: async (method) => { if (method === 'v4/conversation/subscribe') { subscribeCalls += 1; return { ack: { subscriptionId, mode: 'snapshot', logEpoch: `epoch-${index}` } }; } unsubscribeCalls += 1; return { malformed: true }; }, close: async () => { closeCalls += 1; } }; broker.protocol = protocol; const subscribeId = 1_000 + index * 2; await broker.handleLocal(socket, JSON.stringify({ id: subscribeId, method: 'v4/conversation/subscribe', params: { topic: `conversation/${sessionId}`, connectionId, clientMode: 'desktop-continuous' } })); if (writes.find((frame) => frame.id === subscribeId)?.result) await broker.handleLocal(socket, JSON.stringify({ id: subscribeId + 1, method: 'v4/conversation/unsubscribe', params: { topic: `conversation/${sessionId}`, subscriptionId, connectionId } }));
  }
  assert.equal(subscribeCalls, 256); assert.equal(unsubscribeCalls, 256); assert.equal(closeCalls, 256); assert.equal(broker.orphanedConversationSubscriptions.size, 256); assert.equal(broker.conversationSubscriptions.size, 0); assert.equal(broker.admission.activeCount, 0); await rm(directory, { recursive: true, force: true });
});

test('cleanup and retry validate exact empty unsubscribe acknowledgements and retain old-generation evidence', async (t) => {
...
test('owner create epochs fence explicit and anonymous create against release in both orders', async (t) => {
  for (const explicit of [false, true]) await t.test(`create-first-${explicit ? 'explicit' : 'anonymous'}`, { timeout: 3_000 }, async () => {
    const directory = await mkdtemp(join(tmpdir(), 'zcode-broker-create-first-release-')); const endpoint = join(directory, 'broker.sock'); const ownershipPath = `${endpoint}.owners.json`; const ownerId = 'create-first-release-owner'; const createdSessionId = `create-first-${explicit ? 'explicit' : 'anonymous'}-session`; const writes = []; const socket = { writable: true, destroyed: false, zcodeWriter: { write: (line) => writes.push(JSON.parse(line)) }, destroy() {} }; const broker = newTestBroker({ endpoint, ownershipPath, brokerToken: '5'.repeat(64), workspace: directory, launch: { command: process.execPath, args: [fixture], target: fixture } }); broker.authenticated.add(socket); broker.socketOwnerIds.set(socket, ownerId); let resolveCreate; let markCreateEntered; const createEntered = new Promise((resolvePromise) => { markCreateEntered = resolvePromise; }); let createCalls = 0; const protocol = { request: (method) => { if (method !== 'session/create') throw new Error(`unexpected ${method}`); createCalls += 1; markCreateEntered(); return new Promise((resolvePromise) => { resolveCreate = resolvePromise; }); } }; broker.protocol = protocol; const createParams = { workspace: directory, ...(explicit ? { sessionId: createdSessionId } : {}) };
    const creating = broker.handleLocal(socket, JSON.stringify({ id: 50, method: 'session/create', params: createParams })); await createEntered; assert.equal(createCalls, 1); await broker.handleLocal(socket, JSON.stringify({ id: 51, method: 'broker/releaseOwner', params: { excludeSessionIds: [] } })); assert.equal(writes.find((frame) => frame.id === 51)?.result?.deferredSessionCount, 1); resolveCreate({ session: { sessionId: createdSessionId } }); await creating; assert.equal(writes.find((frame) => frame.id === 50)?.result, undefined); assert.equal(broker.sessionOwners.has(createdSessionId), false); assert.equal(broker.admission.activeCount, 0); await assert.rejects(readFile(ownershipPath, 'utf8'), { code: 'ENOENT' }); await rm(directory, { recursive: true, force: true });
  });
