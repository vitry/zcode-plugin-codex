...
test('protocol reset cannot clear an owner release fence before durable ownership commit', async () => {
  ...
  releaseLock(); await holder; const released = await releasing; assert.equal(fencedAfterReset, true); assert.deepEqual(released.releasedSessionIds, [sessionId]); assert.equal(broker.stoppingSessions.has(sessionId), false); await rm(directory, { recursive: true, force: true });
});
