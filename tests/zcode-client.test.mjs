...
test('owner release rejects a stop acknowledgement from a protocol generation reset in the same chunk', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'zcode-broker-release-generation-')); const endpoint = join(directory, 'broker.sock'); const ownershipPath = `${endpoint}.owners.json`; const brokerToken = 'a'.repeat(64); const ownerId = 'release-generation-owner'; const sessionId = 'release-generation-session'; await writeFile(ownershipPath, JSON.stringify({ version: 1, sessions: { [sessionId]: ownerId } })); const broker = await newTestBroker({ endpoint, ownershipPath, brokerToken, workspace: directory, launch: { command: process.execPath, args: [fixture], target: fixture }, env: { ...process.env, FAKE_ZCODE_CONCURRENT_STOP_SUBSCRIBE_BATCH: '1', FAKE_ZCODE_BAD_CONVERSATION_ACK_ONCE: '1' } }).start(); const client = await createZCodeClient({ workspace: directory, brokerEndpoint: endpoint, brokerToken, ownerId });
  try { await broker.getProtocol(); const subscribing = client.subscribeConversation(sessionId, { connectionId: 'release-generation-subscribe', clientMode: 'desktop-continuous' }); const releasing = client.releaseOwner([]); const [subscribeOutcome, releaseOutcome] = await Promise.allSettled([subscribing, releasing]); assert.equal(subscribeOutcome.status, 'rejected'); assert.equal(releaseOutcome.status, 'fulfilled'); assert.deepEqual(releaseOutcome.value.releasedSessionIds, []); assert.deepEqual(releaseOutcome.value.failedSessionIds, [sessionId]); assert.equal(broker.sessionOwners.get(sessionId)?.ownerId, ownerId); assert.equal(JSON.parse(await readFile(ownershipPath, 'utf8')).sessions[sessionId], ownerId); assert.equal(broker.stoppingSessions.has(sessionId), false); }
  finally { await client.close(); await broker.close(); await rm(directory, { recursive: true, force: true }); }
});
...
test('owner release aborts its unlocked winner read after a reset compensation misses the deadline', { timeout: 1_000 }, async () => {
  ...
  const started = Date.now(); await assert.rejects(broker.releaseOwner(socket, ownerId, [], started + 80), { code: 'ZCODE_OWNER_RELEASE_TIMEOUT' }); const elapsed = Date.now() - started; assert.equal(observedSignal?.aborted, true); assert.ok(elapsed < 180, `release exceeded its deadline while reading the compensation winner: ${elapsed}ms`); assert.equal(broker.sessionOwners.get(sessionId)?.ownerId, ownerId); assert.equal(broker.uncertainOwnerReleases.get(sessionId), ownerId); assert.equal(broker.stoppingSessions.has(sessionId), false); await rm(directory, { recursive: true, force: true });
});
...
test('owner operation lease admission is globally bounded and exact-token release is idempotent', async () => {
  ...
  for (let index = 0; index < 256; index += 1) { const sessionId = `owner-operation-bound-${index}`; broker.sessionOwners.set(sessionId, { ownerId, socket, claimToken: null }); leases.push(broker.acquireOwnerOperationLease('session/read', sessionId, ownerId, socket)); } broker.scheduleIdleShutdown(); assert.equal(broker.idleTimer, null); assert.equal(broker.ownerOperationLeaseCount, 256); const overflowSessionId = 'owner-operation-bound-overflow'; broker.sessionOwners.set(overflowSessionId, { ownerId, socket, claimToken: null }); assert.throws(() => broker.acquireOwnerOperationLease('session/read', overflowSessionId, ownerId, socket), { code: 'ZCODE_TURN_ACTIVE' }); broker.releaseOwnerOperationLease(leases[0]); broker.releaseOwnerOperationLease(leases[0]); const replacement = broker.acquireOwnerOperationLease('session/setThoughtLevel', overflowSessionId, ownerId, socket); assert.equal(broker.ownerOperationLeaseCount, 256); for (const lease of leases.slice(1)) broker.releaseOwnerOperationLease(lease); broker.releaseOwnerOperationLease(replacement); assert.equal(broker.ownerOperationLeaseCount, 0); assert.equal(broker.ownerOperationLeases.size, 0); assert.notEqual(broker.idleTimer, null); broker.cancelIdleShutdown(); await rm(directory, { recursive: true, force: true });
});
