...
test('broker validates the complete create snapshot before committing ownership', async (t) => {
  for (const explicit of [false, true]) await t.test(explicit ? 'explicit' : 'anonymous', async () => {
    ...
    const broker = await newTestBroker({ endpoint, ownershipPath, brokerToken, workspace: directory, launch: { command: process.execPath, args: [fixture], target: fixture }, env: { ...process.env, FAKE_ZCODE_BAD_SNAPSHOT: 'missing-workspace', FAKE_ZCODE_SESSION_ID: sessionId } }).start();
...
test('broker release returns at its hard deadline while its tracked continuation keeps owner fences', { timeout: 4_000 }, async () => {
  ...
  await broker.handleLocal(socket, JSON.stringify({ id: 85, method: 'session/create', params: { workspace: directory, sessionId: 'release-fenced-create' } }));
