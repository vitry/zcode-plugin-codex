...
test('owner release aborts its unlocked winner read after a reset compensation misses the deadline', { timeout: scaleTestTimeout(5_000) }, async (t) => {
  ...
  t.after(async () => {
    const cleanupErrors = [];
    const waitForCleanup = async (operation, describeResidual) => {
      let timer;
      try {
