...
test('conversation subscribe accepts additive fields and rejects malformed consumed fields', async () => {
  ...
  const valid = {
    ack: {
      subscriptionId: 'sub-1', mode: 'resume', logEpoch: 'epoch-7',
...
test('conversation unsubscribe accepts additive object fields and rejects a non-object result', async () => {
  // The captured result is {}; this synthetic unit-only field proves projection.
  let unsubscribeResult = { future: { ignored: true } };
...
async function createHealthOnlyServer(endpoint, { brokerToken, instanceId, healthResult, hangHealth = false, hangAuth = false, hangRelease = false, deferRelease = false, releaseTimeouts = 0, authDelayMs = 0, healthDelayMs = 0, onMethod = () => {}, onReleaseReady = () => {} }) {
    ...
    socket.on('data', (chunk) => {
      ...
      while (newline !== -1) {
        ...
        onMethod(frame.method); if (frame.method === 'broker/auth' && frame.params?.token === brokerToken && !hangAuth) { if (authDelayMs) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, authDelayMs); socket.write(`${JSON.stringify({ id: frame.id, result: { authenticated: true, ...(frame.params.existingProtocolOnly === true ? { existingProtocolOnly: true } : {}) } })}\n`); }
          ...
          releaseCalls += 1;
          const respond = () => socket.write(`${JSON.stringify(releaseCalls <= releaseTimeouts
            ? { id: frame.id, error: { code: -32_000, message: 'bounded owner release timed out', data: { pluginError: { code: 'ZCODE_OWNER_RELEASE_TIMEOUT', category: 'timeout', remedy: 'Retry after the active owner-store operation completes.', details: {} } } } }
            : { id: frame.id, result: { releasedSessionIds: [], failedSessionIds: [], deferredSessionCount: 0 } })}\n`);
...
test('runtime model update accepts additive result fields and rejects malformed consumed fields', async () => {
...
test('runtime model update rejects fake CLI consumed-field violations', async (t) => {
  for (const variant of ['wrong-session', 'wrong-revision', 'bad-changed']) await t.test(variant, () => withClient(async (client) => {
    const sessionId = (await client.createSession({ workspace: '/repo' })).session.sessionId;
...
test('applied model and thought responses must exactly match before send', async () => {
  ...
  await withClient(async (client) => {
    const { session: { sessionId } } = await client.createSession({ workspace: '/repo' });
    // `extra` is a unit-only additive-field projection probe.
    const result = await client.setModel(sessionId, { providerId: 'fake2', modelId: 'other' });
    assert.deepEqual(result.settings.model.current, { providerId: 'fake2', modelId: 'other', extra: true });
...
test('managed broker subscribe and unsubscribe accept additive upstream fields', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'zcode-broker-conversation-additive-'));
  const ownerId = 'conversation-additive-owner'; const sessionId = 'conversation-additive-session';
  const topic = `conversation/${sessionId}`; const subscriptionId = 'conversation-additive-subscription'; const connectionId = 'conversation-additive-connection';
  ...
  // Preserve the ZCode 0.16.5 bundle's openTiming placement and cold-session
  // fields. The unrelated `future` values are unit-only projection probes.
  const protocol = { request: async (method) => method === 'v4/conversation/subscribe'
    ? { ack: { subscriptionId, mode: 'snapshot', logEpoch: 'epoch-additive', openTiming: { version: 1, initialFrameEncodeMs: 1, sessionRuntimeState: 'cold', snapshotRowCount: 0 } }, future: { opaque: true } }
    : { future: { opaque: true } } };
...
test('three hundred malformed unsubscribe acknowledgements cannot spawn overlapping retired generations', async () => {
  ...
  const protocol = { request: async (method, params) => { if (method === 'v4/conversation/subscribe') { subscribeCalls += 1; return { ack: { subscriptionId: `malformed-unsubscribe-${params.topic}`, mode: 'snapshot', logEpoch: 'malformed-epoch' } }; } unsubscribeCalls += 1; return []; }, close: () => { closeCalls += 1; return new Promise(() => {}); } }; broker.protocol = protocol;
...
test('owner release keeps its stop fence through durable ownership commit', async () => {
  ...
  const releasing = broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000); await new Promise((resolvePromise) => setTimeout(resolvePromise, 40)); const fencedDuringCommit = broker.stoppingSessions.has(sessionId); const sending = broker.handleLocal(socket, JSON.stringify({ id: 91, method: 'session/send', params: { sessionId, inputId: 'new-input', content: 'must reject' } })); const reading = broker.handleLocal(socket, JSON.stringify({ id: 92, method: 'session/read', params: { sessionId } })); await new Promise((resolvePromise) => setTimeout(resolvePromise, 20)); assert.equal(sendCalls, 0); assert.equal(readCalls, 0);
...
test('owner release fences a session without an existing protocol through durable ownership commit', async () => {
  ...
  let releaseLock; let lockEntered; const gate = new Promise((resolvePromise) => { releaseLock = resolvePromise; }); const entered = new Promise((resolvePromise) => { lockEntered = resolvePromise; }); const holder = withFileLock(`${ownershipPath}.lock`, async () => { lockEntered(); await gate; }); await entered; const releasing = broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000); await new Promise((resolvePromise) => setTimeout(resolvePromise, 40)); const fencedDuringCommit = broker.stoppingSessions.has(sessionId);
...
test('protocol reset invalidates an owner release acknowledgement and clears its exact fence', async () => {
  ...
  let releaseLock; let lockEntered; const gate = new Promise((resolvePromise) => { releaseLock = resolvePromise; }); const entered = new Promise((resolvePromise) => { lockEntered = resolvePromise; }); const holder = withFileLock(`${ownershipPath}.lock`, async () => { lockEntered(); await gate; }); await entered; const releasing = broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000); await new Promise((resolvePromise) => setTimeout(resolvePromise, 40)); const fencedAfterReset = broker.stoppingSessions.has(sessionId);
...
test('owner release fails closed when stale sibling compensation fails before restoring authority', async () => {
  ...
  await assert.rejects(broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000), (error) => error === compensationError); assert.deepEqual(JSON.parse(await readFile(ownershipPath, 'utf8')).sessions, {}); assert.equal(broker.sessionOwners.get(sessionId)?.ownerId, ownerId); assert.equal(broker.sessionOwners.get(siblingId)?.ownerId, ownerId); assert.equal(broker.uncertainOwnerReleases.get(sessionId), ownerId); assert.equal(broker.uncertainOwnerReleases.get(siblingId), ownerId); await rm(directory, { recursive: true, force: true });
});
...
test('owner release aborts its unlocked winner read after a reset compensation misses the deadline', { timeout: scaleTestTimeout(3_000) }, async () => {
  ...
  try {
    ...
    assert.equal(observedSignal?.aborted, true); assert.equal(broker.sessionOwners.get(sessionId)?.ownerId, ownerId); assert.equal(broker.sessionOwners.get(siblingId)?.ownerId, ownerId); assert.equal(broker.uncertainOwnerReleases.get(sessionId), ownerId); assert.equal(broker.uncertainOwnerReleases.get(siblingId), ownerId); assert.equal(broker.stoppingSessions.has(sessionId), false); assert.equal(broker.stoppingSessions.has(siblingId), false);
...
test('owner release recovers an atomic owner commit that applied before throwing', async () => {
  ...
  const released = await broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000); assert.equal(writeAttempts, 1); assert.deepEqual(released.releasedSessionIds, [sessionId]); assert.equal(broker.sessionOwners.has(sessionId), false); assert.deepEqual(JSON.parse(await readFile(ownershipPath, 'utf8')).sessions, {}); await rm(directory, { recursive: true, force: true });
});
...
test('an unresolved owner commit stays fail-closed and reconciles its durable winner on the next reload', async () => {
  ...
  await assert.rejects(broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000), (error) => error === commitError); assert.equal(broker.sessionOwners.get(sessionId)?.ownerId, ownerId); assert.deepEqual(JSON.parse(await readFile(ownershipPath, 'utf8')).sessions, {}); broker.readOwnerStore = readOwnerStore; broker.readOwnerStoreUnlocked = readOwnerStoreUnlocked; await broker.reloadOwnership(); assert.equal(broker.sessionOwners.has(sessionId), false); await rm(directory, { recursive: true, force: true });
});
...
test('broker release returns at its hard deadline while its tracked continuation keeps owner fences', { timeout: 4_000 }, async () => {
  ...
  const retried = await broker.releaseOwner(socket, ownerId, [], Date.now() + 2_000); assert.deepEqual(retried.releasedSessionIds, [sessionId]); assert.equal(stopCalls, 1); assert.equal(broker.sessionOwners.has(sessionId), false); await rm(directory, { recursive: true, force: true });
});
...
test('managed owner cleanup retries a bounded broker release timeout within its shared deadline', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'zcode-managed-release-timeout-retry-')); const methods = []; let closeServer;
  try {
    const storage = await resolveWorkspaceStorage({ dataRoot: directory, workspace: directory }); const endpoint = brokerEndpointFor({ dataRoot: directory, workspace: storage.workspacePath }); const record = { endpoint, pid: process.pid, instanceId: 'd'.repeat(48), brokerToken: 'e'.repeat(64) }; closeServer = await createHealthOnlyServer(endpoint, { ...record, releaseTimeouts: 1, onMethod: (method) => methods.push(method) }); await writeBrokerIdentity(join(storage.directory, 'broker', 'identity.json'), record);
    assert.deepEqual(await releaseManagedZCodeOwner({ dataRoot: directory, workspace: directory, ownerId: 'managed-release-timeout-retry-owner', requestTimeoutMs: 500 }), { releasedSessionIds: [], failedSessionIds: [], deferredSessionCount: 0 });
    assert.deepEqual(methods, ['broker/auth', 'broker/health', 'broker/releaseOwner', 'broker/releaseOwner']);
  } finally { await closeServer?.(); await rm(directory, { recursive: true, force: true }); }
});

test('managed owner cleanup fails closed when broker release timeouts exhaust its shared deadline', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'zcode-managed-release-timeout-exhausted-')); const methods = []; let closeServer;
  try {
    const storage = await resolveWorkspaceStorage({ dataRoot: directory, workspace: directory }); const endpoint = brokerEndpointFor({ dataRoot: directory, workspace: storage.workspacePath }); const record = { endpoint, pid: process.pid, instanceId: 'f'.repeat(48), brokerToken: '1'.repeat(64) }; closeServer = await createHealthOnlyServer(endpoint, { ...record, releaseTimeouts: 100, onMethod: (method) => methods.push(method) }); await writeBrokerIdentity(join(storage.directory, 'broker', 'identity.json'), record); const started = Date.now();
    await assert.rejects(releaseManagedZCodeOwner({ dataRoot: directory, workspace: directory, ownerId: 'managed-release-timeout-exhausted-owner', requestTimeoutMs: 500, cleanupBudgetMs: 100 }), (error) => error?.code === 'ZCODE_OWNER_RELEASE_INCOMPLETE' && error.details?.causeCodeCounts?.ZCODE_OWNER_RELEASE_TIMEOUT === 1);
    assert.ok(Date.now() - started < scaleTestTimeout(500)); assert.ok(methods.filter((method) => method === 'broker/releaseOwner').length >= 2);
  } finally { await closeServer?.(); await rm(directory, { recursive: true, force: true }); }
});
