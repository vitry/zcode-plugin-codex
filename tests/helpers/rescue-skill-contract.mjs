...
export const expectedGenericRescueInstruction = "For the generic route, substitute only the already-bound immutable `rescueLauncherPath` for `<rescue-launcher-path>` in this fixed message, then call `spawn_agent` with `task_name: rescueTaskName`, `fork_turns: 'none'`, no `agent_type`, and exactly that message:";
...
export function assertRescueLauncherGate(source, { assertionPrefix = '' } = {}) {
  ...
  assert.match(block, /\[zcode-rescue-launcher\][^\n]+"version":1[^\n]+"launcherPath":"<absolute path>"/i);
  assert.match(block, /bind `rescueLauncherPath` exactly once/i);
