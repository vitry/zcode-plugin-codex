...
test('SessionEnd never starts a broker when exact existing settlement is unavailable', async () => {
  ...
  assert.equal(ended.code, 0, ended.stderr); const archived = await store.readJob(cwd, value.id); assert.equal(archived.status, 'failed'); assert.match(archived.error.message, /SessionEnd.*control channel.*unavailable.*orphan/i); await assert.rejects(readFile(record, 'utf8'), { code: 'ENOENT' }); await assert.rejects(identity.resolveActiveTurn({ sessionId: 'absent-owner', workspace: cwd }), { code: 'ACTIVE_TURN_NOT_FOUND' });
...
test('SessionEnd archives a job when a reachable broker has no existing protocol and never lazily spawns ZCode', async () => {
  ...
  const ended = await runHook('session-end-hook.mjs', { session_id: ownerSessionId, cwd, hook_event_name: 'SessionEnd', transcript_path: null, reason: 'other' }, env); assert.equal(ended.code, 0, ended.stderr); const archived = await store.readJob(cwd, value.id); assert.equal(archived.status, 'failed'); assert.match(archived.error.message, /SessionEnd.*control channel.*unavailable.*orphan/i); assert.equal(await readFile(record, 'utf8'), '');
});
