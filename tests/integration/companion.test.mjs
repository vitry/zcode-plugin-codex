...
test('a new owner scavenges one orphan blocker and retries writable reservation exactly once', async () => {
  const context = await fixture(); const { job: orphan, store } = await reserveOrphan(context); const calls = [];
...
test('two new owners racing through scavenging admit at most one writable rescue', async () => {
  ...
  assert.equal((await store.readJob(context.workspace, orphan.id)).status, 'failed');
  const activeWritable = (await store.listJobs(context.workspace)).filter((job) => ['queued', 'running', 'cancelling'].includes(job.status) && !job.readOnly);
...
test('status --all reports a scavenged foreign job only through redacted other-owner metadata', async () => {
  ...
  const foreign = listed.jobs.find((job) => job.id === orphan.id); assert.ok(foreign); assert.equal(foreign.owned, false); assert.equal(foreign.owner, 'other');
