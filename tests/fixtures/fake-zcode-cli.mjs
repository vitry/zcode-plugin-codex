...
function messages(sessionId, model = defaultModel) { return resultMessages(sessionId, model, false); }
function resultMessages(sessionId, model, review) {
  const mode = process.env.FAKE_ZCODE_RESULT_MODE ?? 'text';
  const structured = review ? mode === 'invalid-structured' ? { findings: [{ severity: 'bogus' }] } : { findings: [] } : undefined;
  const base = { partId: 'part-assistant', sessionId, messageId: 'message-assistant-1' };
  ...
  return [{ info: { messageId: 'message-user-1', sessionId, role: 'user', time: { created: 1, completed: 2 }, agent: 'build', model, synthetic: false, visibility: 'user-visible' }, parts: [{ partId: 'part-user-1', sessionId, messageId: 'message-user-1', type: 'text', text: 'hello' }] }, { info: { messageId: 'message-assistant-1', sessionId, role: 'assistant', time: { created: 2, completed: 3 }, parentMessageId: 'message-user-1', agent: 'build', model, path: { cwd: '/repo', root: '/repo' }, cost: 0, tokens: { input: 1, output: 1, reasoning: 0, cache: { read: 0, write: 0 } }, finish: 'stop', ...(structured === undefined ? {} : { structured }) }, parts }];
}
...
input.on('line', async (line) => {
  ...
  switch (message.method) {
    ...
    case 'session/send': {
      sendCount += 1;
      const session = sessions.get(p.sessionId); if (session) session.messages = resultMessages(p.sessionId, session.settings.model.current, /strictly read-only|code review/i.test(p.content));
