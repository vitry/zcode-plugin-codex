...
const legacyAdoptActivation = Object.freeze({
  kind: 'legacy-adopt', childThreadId: 'legacy-child', agentPathDigest: 'c'.repeat(64),
});
const legacyBoundActivation = Object.freeze({
  kind: 'legacy-bound', childThreadId: 'legacy-child', agentPathDigest: 'c'.repeat(64), bindingKey: 'd'.repeat(64),
});
...
test('v3 rejects pair-shaped and non-resume continuation targets', async () => {
  ...
  for (const value of [
    ...
    { ...exact, continuationTarget: { agentPath: 7 } },
    { ...exact, continuationTarget: { agentPath: 'root/child' } },
    { ...exact, continuationTarget: { agentPath: '/root/../child' } },
    { ...exact, continuationTarget: { agentPath: '/root/child\n' } },
    { ...exact, continuationTarget: { agentPath: `/root/${'x'.repeat(1019)}` } },
    { ...exact, continuationTarget: '/root/zcode_rescue_task_2' },
    { ...exact, continuationTarget: 7 },
    { ...exact, continuationTarget: ['/root/zcode_rescue_task_2'] },
