...
After mid-turn context compaction, `SessionStart` with source `compact` restores that same instance-bound launcher descriptor within its 1200-byte context limit. It preserves the already active turn and grants no new authority. Ordinary `startup`, `resume`, and `clear` SessionStart sources keep only the generic lifecycle context; the following `UserPromptSubmit` supplies the ordinary turn descriptor. Unsafe provenance emits only the fixed `[zcode-rescue-launcher-error]` context and never falls back to another installation.

Rescue distinguishes the conversation's origin workspace from its execution workspace. When Root creates or enters a linked worktree during the same parent turn, the first trusted `prepare rescue` automatically binds execution there; no manual handoff is needed. The origin itself or a canonical linked-worktree top level is eligible only when it shares the same canonical Git common-dir. That target is immutable for the turn, so another worktree or an unrelated repository is rejected. Role inspection is read-only and a child cannot claim or change the target. Root `Stop` and a new prompt revoke or replace authority before cleanup across the origin and bound target. `SessionEnd` means runtime ownership loss: it settles writable jobs and cleans runtime/preparation resources while preserving exact resumable bindings.
...
`--resume` remains an argument-free public choice: it accepts no child, session, job, binding, operation, or handle argument. For an exact continuation, Root privately retains the exact `task_name` returned by its successful `spawn_agent` call as the operation's unchanged canonical agent path. Root sends a private version-3 preparation containing only that path as `continuationTarget: {"agentPath":"/root/..."}`; the plugin resolves the host child ID internally from the exact parent's persisted child graph. The canonical path is a selector that only narrows selection, not authority: complete child-ID/path binding, original session, permission, and canonical workspace validation remain mandatory. When multiple eligible siblings exist, the exact version-3 selector resolves only the sibling at that canonical agent path and resumes only that sibling's complete binding and original ZCode session. The targetless/null compatibility route is allowed only when exactly one complete binding is eligible. Private envelope version 2's child ID/path pair remains read compatibility only for already-created preparations; new flows never emit it. A targetless resume with multiple usable bindings is still ambiguous and must fail closed; the plugin never ranks or substitutes a sibling.

Root starts `prepare rescue` on a raw-capable TTY. The companion enables raw mode before it emits the exact task-free readiness line; readiness is nonterminal. Only after that line does Root send one JSON line terminated by LF over private stdin, with no EOF or U+0004. The companion consumes that one frame, restores raw mode, and commits exact session, turn, workspace, and executor-bound prepared state. Non-TTY or raw-mode failure stops before task delivery and no child is spawned. Tool output never contains or echoes the payload; only the task-free readiness and final prepared acknowledgement cross back. The named Role and generic child then run the same constant `invoke-prepared rescue` forwarder, without receiving the task, options, capability, or authorization material. If an active `rescueChildPath` already exists, it is the exact canonical `task_name` returned by the successful `spawn_agent` call; Root rejoins and waits for the Rescue child at that path instead of repeating preflight, preparation, spawn, or invocation. Root never receives or retains the child ID; the plugin resolves it internally from the exact parent's persisted child graph.

The Companion discovers persisted Codex children only through the app-server's exact parent relationship query. An existing child is eligible only through its complete exact binding; host-only or jobs-only history is never adopted. Lazy compatibility migration is limited to one exact v1/v2 `closed/session-ended` binding. A separately observed `notLoaded` child is accepted only when parent, child ID, exact path, Role/type, canonical workspace, binding, jobs, operation, generation, permission, and original non-empty `zcodeSessionId` all agree. Thus, with an exact private selector, `/root/zcode_rescue_task_2` resumes only its own session even when multiple eligible sibling bindings exist; only the targetless route requires it to be the sole eligible binding. Without an exact private selector, two usable bindings are ambiguous and fail closed. Base path, latest job/session, timestamps, list order, and suffix rank never select a candidate. Follow-up directives remain strict version two with a fixed assignment; spawn directives remain strict version one without an assignment.

A durable Rescue binding keeps one exact stopped Rescue child attached to one exact ZCode session. Its private `anchorJobId` identifies the operation, and `currentJobId` advances when each continuation job is durably reserved and published, even if that job later queues, fails, or is cancelled; neither identifier is sent in a child message. Resume follows up that same child with no second `SubagentStart`, calls `session/resume` for the original non-empty `zcodeSessionId`, and never creates or substitutes a session. `--fresh` always uses a newly spawned independent Codex child and one new ZCode session. A pending fresh choice first returns the exact `parent-replan` outcome, retires the old child selection, and makes the parent prepare and collision-free spawn the new child. Fresh never performs same-child replacement, reactivate, or follow up, and leaves existing sibling bindings unchanged.

Ordinary foreground Rescue completion has no plugin-defined wall-clock deadline. Active parent authority is lifecycle-bound rather than time-bound, so a clear continuation in that still-active parent turn can replace the consumed one-shot preparation with the next 30-minute generation, follow up the exact stopped child, and reuse the exact bound ZCode session. Caller credentials and every preparation generation remain bounded to 30 minutes: that TTL is only the one-shot capability window in which a prepared child may start, not a lifetime for the Codex child, the Rescue binding, or the ZCode operation. Request RPCs, the optional Stop review gate, qualification harnesses, and explicit status waits also retain finite budgets. Root `Stop`, a replacement `UserPromptSubmit`, `$zcode:cancel`, `SIGINT`, and `SIGTERM` remain authoritative termination or revocation boundaries; `SessionEnd` only removes runtime ownership and preserves exact resumable bindings.

Only the exact v1/v2 `closed/session-ended` compatibility record can migrate; v3 records use normal exact resume. Jobs-only state, host-only children, revoked records, and incomplete or contradictory evidence cannot migrate. Explicit cancel and invalidation remain terminal. Every mismatch fails closed without base/latest fallback, fresh fallback, mutation, or ZCode RPC. Because the managed Role bytes changed, `role-status rescue` can report `upgrade-required`; rerun `$zcode:setup` before continuing.
...
Cold resume has one narrow compatibility recovery. Only after `session/resume` returns the exact snapshot warning `ZCODE_RUNTIME_MODEL_UNAVAILABLE` does the plugin lazily read the effective `HOME` file `.zcode/cli/config.json`. It resolves a complete bounded runtime configuration for the tuple selected by explicit `--model`, workspace model policy, or CLI `model.main`, in that precedence order. The in-memory runtime may include the configured provider adapter and credential, and travels only over the authenticated local broker path; it is never cached, logged, persisted, or rendered. The plugin calls `session/updateRuntimeModelConfig` exactly once, performs one `session/read` to require the warning to be gone and the exact tuple to be current, then applies requested effort and performs one `session/send`. A warm resume does not read config or issue a runtime update. Missing or invalid config remains the bounded original runtime-unavailable failure; a genuine AppServer runtime-update rejection and a later send failure remain authoritative. There is no resume retry, fresh fallback, prompt resend, replacement session, retry loop, or CLI-config mutation. Ordinary non-cold explicit model selection still uses `session/setModel`.
...
`SessionEnd` is runtime ownership loss. It preserves only an exact completed binding with no active current attempt and an exact v1/v2 `closed/session-ended` candidate as resumable. After confirmed remote cancellation, it may close only that exact active operation; an unacknowledged stop retains the writable guard. Immediately before any bound `session/stop`, the stale-stop final guard rechecks owner, cancellable job, binding operation/generation/current job, and worker lease; a stale loser sends no stop and closes nothing.
...
Direct `$zcode:status`, `$zcode:result`, and `$zcode:cancel` use one lifecycle-authoritative current job partition. From either the eligible origin workspace or the exact bound execution target, they operate on that selected target partition, which is preserved privately across later turns. Review, adversarial-review, and transfer select their actual current partition before persisting or reserving work. The plugin never scans or merges workspace partitions: latest selection, `--all`, logs, reconciliation, cancellation, and binding closure all use the same partition. An explicit job ID does not grant cross-partition or owner authority, and no command gains a workspace argument. The existing limit of one active writable Rescue per canonical workspace is unchanged.
...
If a foreground or background response is lost after ZCode may have accepted the turn, recovery uses the same durable job through `$zcode:status`, `$zcode:result`, owned-job reconciliation, the persisted session read, and its result artifact. Recovery preserves one send: it never automatically resends the prompt, calls `session/create`, rolls back the accepted turn, or interprets response loss as fresh.
