...
The per-job log may also store current-turn visible assistant text selected by the exact existing linkage rules and the authoritative final output. Raw command stdout/stderr, arbitrary tool input/output/errors/metadata, file contents, patches, environment values, credentials, capabilities, hidden messages, and raw reasoning remain excluded. Logs and progress are observational and cannot establish or alter terminal authority.
