...
Rescue children use task-specific native display names like `/root/zcode_rescue_fix_progress`, with a bounded ordinal on sibling collision. Names and paths are for navigation only: matching `zcode_rescue_*` neither proves Rescue nor grants authority; a different display name does not remove authority from an otherwise trusted Rescue child.
