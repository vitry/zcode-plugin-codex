...
- Backup and rollback location: `/Users/zhangzikai/.codex/plugins/backups-cancellation-incident-20260915/` with `manifest.sha256`. Rollback procedure: under the same state locks, first verify the post-repair hashes are still unchanged since the repair and that no successor binding generation, new writable job, or newer lifecycle evidence exists; only then restore the backed-up job JSON. Any such change refuses rollback and reconciles instead — never restore over concurrent activity.
