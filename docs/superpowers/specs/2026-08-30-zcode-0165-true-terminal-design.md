...
Historical or replayed terminal rows cannot finish the current turn. The subscribe acknowledgement alone does not establish history: the observer must wait for a complete initial or recovery snapshot baseline before it can be armed. Online deltas received before that baseline cannot grant turn authority; if ordering or recovery cannot establish the baseline, v4 authority is disabled and the coordinator uses snapshot fallback. After the baseline is established, the observer is armed immediately before `session/send`; it must correlate a terminal row with a new `turnHeader` first observed after that boundary. The accepted input boundary is then persisted before any terminal candidate may be published. Recovery/snapshot frames may restore lifecycle continuity but cannot silently substitute an unrelated historical turn.
...
An empty successful `session/stop` response is not sufficient cancellation proof during the 0.16.5 admission/start gap. The existing cancellation lock fences the complete admission publication sequence: revalidate the running job, send the prompt, and persist the accepted input boundary before cancellation may observe that job. A job with no durable accepted boundary, or whose boundary publication failed after send, cannot be terminalized from a bare stop acknowledgement by foreground cleanup, cancellation, SessionEnd, or orphan recovery; its writable guard is retained for later exact reconciliation. Once a boundary exists, a job remains `cancelling` while current-turn evidence is unresolved. While holding the same lock, the managed client performs bounded v4/snapshot observation; if the current turn later appears active, cancellation issues another guarded stop. The job becomes `cancelled` only after coherent interrupted/failed evidence. A completed-success result retains the existing terminal-winner semantics. If bounded observation expires or the protocol remains uncertain, the writable guard is preserved and cancellation reports `JOB_CANCEL_FAILED`, so a later cancel or recovery can retry rather than falsely terminalizing the job. No background daemon is introduced.
