...
Placement-split amendment (2026-09-16): the no-flag inference is a Root-side decision made before preparation, not a Companion choice. It fixes Host placement to `foreground` and selects only Companion execution by task complexity: small, clearly bounded work stays Companion foreground, while complex, open-ended, multi-step, or likely long-running work selects Companion background, whose Rescue Child returns queued after starting the session-bound detached runner. Host background is reserved for the explicit `--background` flag and carries attached Companion foreground; it never selects the detached runner. This amendment re-scopes the inference above without changing its SessionEnd authority, durable reconciliation, or coordination-loss rules.
