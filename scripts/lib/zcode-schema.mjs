...
export function validSessionInfo(/** @type {any} */ value) {
  ...
  return text(value.sessionId) && validWorkspace(value.workspace) && SESSION_KINDS.includes(value.sessionKind) && string(value.title) && MODES.includes(value.mode) && STATUSES.includes(value.status) && uint(value.createdAt) && uint(value.updatedAt)
...
function basePart(/** @type {any} */ value, /** @type {string[]} */ required, /** @type {string[]} */ optionalKeys = []) { return exact(value, ['partId', 'sessionId', 'messageId', 'type', ...required], optionalKeys) && text(value.partId) && text(value.sessionId) && text(value.messageId); }
/** VNe */
