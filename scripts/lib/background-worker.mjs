...
/** Start the private worker, deliver its capability internally, and detach only after a bounded acknowledgement. @param {{companionPath:string,jobId:string,executionCapability:string,cwd:string,env?:NodeJS.ProcessEnv,timeoutMs?:number}} input */
export async function startBackgroundWorker({ companionPath, jobId, executionCapability, cwd, env, timeoutMs = 5_000 }) {
  ...
  try {
    await new Promise((resolve, reject) => {
      timer = setTimeout(() => reject(workerError('BACKGROUND_WORKER_START_TIMEOUT', 'Background worker did not acknowledge startup.')), timeoutMs);
