...
export async function collectGitFacts(input) {
  ...
  if (input.base?.startsWith('-')) throw gitError('GIT_BASE_INVALID', 'Git base refs cannot start with "-".');
  /** @param {...string} args */
  ...
  if (input.base) await run('rev-parse', '--verify', '--end-of-options', `${input.base}^{commit}`);
...
async function defaultBase(run) {
  for (const ref of ['origin/HEAD', 'origin/main', 'origin/master', 'main', 'master']) {
    try { await run('rev-parse', '--verify', '--end-of-options', `${ref}^{commit}`); return ref; } catch { /* try next */ }
  }
  return 'HEAD';
}
