...
export class ZCodeClient {
  ...
  async createSession(input) {
    ...
    if (!plainObject(result) || !plainObject(result.session) || !nonEmpty(result.session.sessionId)) throw outputError('session/create');
  ...
  async send(sessionId, content, options = {}) {
    ...
    const result = await this.protocol.request('session/send', { sessionId, content });
    if (!plainObject(result) || result.accepted !== true || result.sessionId !== sessionId) throw outputError('session/send');
    return result;
  }

  /** @param {string} sessionId */ async readSession(sessionId) { requireString(sessionId); return requireObjectResult(await this.protocol.request('session/read', { sessionId }), 'session/read'); }
  /** @param {string} sessionId */ async resumeSession(sessionId) { requireString(sessionId); return requireObjectResult(await this.protocol.request('session/resume', { sessionId }), 'session/resume'); }
  async listSessions() { const result = requireObjectResult(await this.protocol.request('session/list', {}), 'session/list'); if (!Array.isArray(result.sessions)) throw outputError('session/list'); return result; }
  /** @param {string} sessionId */ async stopSession(sessionId) { requireString(sessionId); return requireObjectResult(await this.protocol.request('session/stop', { sessionId }), 'session/stop'); }
  ...
  setModel(sessionId, model) {
    ...
    return this.protocol.request('session/setModel', { sessionId, model: copyModel(model), persistAsWorkspaceLastUsed: false });
  }
  ...
  async setThoughtLevel(sessionId, thoughtLevel) {
    ...
    const advertised = advertisedThoughtLevels(selected).map((value) => value.toLowerCase());
    if (!advertised.includes(normalized)) throw new PluginError('ZCODE_THOUGHT_LEVEL_UNSUPPORTED', 'The selected model does not advertise this thought level.', { category: 'configuration', remedy: 'Choose a thought level advertised by the selected model.', details: { thoughtLevel: normalized } });
    return this.protocol.request('session/setThoughtLevel', { sessionId, thoughtLevel: normalized, persistAsWorkspaceLastUsed: false });
  }
...
/** @param {{workspace:string,launch?:{command:string,args:string[],target?:string},brokerEndpoint?:string,brokerToken?:string,env?:NodeJS.ProcessEnv,requestTimeoutMs?:number,completionTimeoutMs?:number,maxFrameBytes?:number}} options */
export async function createZCodeClient(options) {
  ...
  const protocol = options.brokerEndpoint
    ? await connectZCodeBroker(options.brokerEndpoint, { ...protocolOptions, brokerToken: /** @type {string} */ (options.brokerToken) })
...
/** @param {{dataRoot:string,workspace:string,launch:{command:string,args:string[],target?:string},env?:NodeJS.ProcessEnv,requestTimeoutMs?:number,completionTimeoutMs?:number,maxFrameBytes?:number}} options */
export async function createManagedZCodeClient(options) {
  ...
  return createZCodeClient({ workspace: options.workspace, brokerEndpoint: identity.endpoint, brokerToken: identity.brokerToken, requestTimeoutMs: options.requestTimeoutMs, completionTimeoutMs: options.completionTimeoutMs, maxFrameBytes: options.maxFrameBytes });
}
